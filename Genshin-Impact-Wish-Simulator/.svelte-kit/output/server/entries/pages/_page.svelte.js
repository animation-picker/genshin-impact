...
const Header = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  const previousClick = () => {
    ...
    }).then((response) => response.json()).then((data2) => console.log(data2)).catch((error) => {
  ...
  return `<div id="${"header"}"${add_attribute("style", headerHeightstyle, 0)} class="${"svelte-10cweyq"}"><div class="${"top svelte-10cweyq"}"><h1 class="${"wish-title svelte-10cweyq"}"><img${add_attribute("src", $assets["brand.png"], 0)} alt="${"Brand"}" crossorigin="${"anonymous"}" class="${"svelte-10cweyq"}">
			...
			${!$editorMode ? `<span>Wish </span>` : `<span>${escape($t("customBanner.title"))}</span>`}

			<button class="${"help svelte-10cweyq"}" title="${"Setting"}" aria-label="${"Setting"}"><i class="${"gi-help"}"></i></button>
			

			${!$isPWA || !$isMobile ? `<button class="${"fullscreen svelte-10cweyq"}" title="${"FullScreen"}" aria-label="${"Fullscreen"}"><i class="${"gi-" + escape("shrink", true)}"></i></button>` : ``}</h1>
    ...
    default: () => {
      return `${escape(unlimitedWish ? "\u221E" : balance)}`;
				...
				<button class="${"close"}" title="${"Change Banner"}"><i class="${"gi-close"}"></i></button>` : `<button class="${"close"}" title="${"Cancel Edit"}"><i class="${"gi-close"}"></i></button>`}</div></div>
